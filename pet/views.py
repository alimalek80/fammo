                ...
                try:
                    ...
                    try:
                        send_mail(
                            ...
                            fail_silently=False,  # Show email errors
                        )
                        messages.success(self.request, f"📧 Test report for {pet_name} has been sent to {user_email}!")
                        print(f"✅ Test report email sent to {user_email}")  # Debug log
                    except Exception as email_error:
                        messages.error(self.request, f"We couldn't send the report. Please try again later. Error: {str(email_error)}")
                        print(f"❌ Test report email error for {user_email}: {email_error}")  # Debug log
                    
                    return redirect('/')
                    
                except Exception as e:
                    print(f"❌ Test report error: {e}")  # Debug log
